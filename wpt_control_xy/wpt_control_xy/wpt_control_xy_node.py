...
from geometry_msgs.msg import Vector3Stamped, PoseStamped, Pose, Twist
...
class WptControl(Node):
    ...
    def __init__(self):
        ...
        self.vector_pub_ = self.create_publisher(Twist, 'cmd_vel', 10)
        ...
        self.pos_sub_ = self.create_subscription(Odometry, 'pose', self.pose_callback, qos_profile=qos_profile_sensor_data)
        ...
        if self.init_gps and self.init_wpt:
            ...
            if self.distance_remaining > self.capture_rad:
                if not self.mission_pause:
                    ...
                    x_error = x2-x1
                    y_error = y2-y1
                    den = np.sqrt(x_error**2 + y_error**2)
                    x_error = (x2-x1)/den
                    y_error = (y2-y1)/den
                    
                    #ang = max(min(tmp, self.max_ang), -self.max_ang)
                    self.msg.linear.x = x_error 
                    self.msg.linear.y = y_error
